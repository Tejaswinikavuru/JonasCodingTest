...
namespace UnitTests
{
    ...
    public class EmployeeServiceTests
    {
        ...
        {
            // Arrange
            var employees = new List<Employee> { new Employee { EmployeeCode = "E1" } };
            var employeeInfos = new List<EmployeeInfo> { new EmployeeInfo { EmployeeCode = "E1" } };
            _mockEmployeeRepository.GetAllEmployeesAsync().Returns((IEnumerable<Employee>)Task.FromResult(employees));
        ...
        {
            // Arrange
            var employeeInfo = new EmployeeInfo { EmployeeCode = "E1" };
            var employee = new Employee { EmployeeCode = "E1" };
            var resultInfo = new DbResultInfo { IsSuccess = true, Message = "Employee created successfully." };
        ...
        public async Task UpdateEmployeeByCodeAsync_ShouldUpdateEmployeeSuccessfully_WhenFound()
        {
            ...
            var employeeInfo = new EmployeeInfo { EmployeeCode = employeeCode };
            var employee = new Employee { EmployeeCode = employeeCode };
            var updateResult = new DbResultInfo { IsSuccess = true, Message = "Employee updated successfully." };
        ...
        public async Task UpdateEmployeeByCodeAsync_ShouldReturnFailure_WhenNotFound()
        {
            ...
            var employeeInfo = new EmployeeInfo { EmployeeCode = employeeCode };
            var employee = new Employee { EmployeeCode = employeeCode };
            var updateResult = new DbResultInfo { IsSuccess = false, Message = "Failed to update employee." };
